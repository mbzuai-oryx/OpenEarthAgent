...
logger = build_logger(__file__, f"RegionAttributeDescription_worker_{worker_id}.log")

class RegionAttributeDescriptionWorker(BaseToolWorker):
    def __init__(self,
                 controller_addr,
                 worker_addr="auto",
                 worker_id=worker_id,
                 no_register=False,
                 model_name="RegionAttributeDescription",
                 device="cuda",
    ...
    def generate(self, params):
        ...
        try:
            ...
            # send to BaseModel
            payload = {"image": img_b64, "query": f"Describe {attribute} in this aerial image in detail."}
            headers = {"User-Agent": "RegionAttributeDescriptionWorker"}
            resp = requests.post(
            ...
            if resp.get("error_code", 1) != 0:
                ...
                return {"text": resp, "error_code": 1}

            txt = resp["text"]
            return {"text": txt, "error_code": 0}

        except Exception as e:
            txt_e = f"Error in RegionAttributeDescription: {e}"
            logger.error(txt_e)
    ...
    def get_tool_instruction(self):
        return {
            ...
            "function": {
                ...
                "description": "Describe a specific attribute of an object in a region of the image using BaseModel.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "image": {"type": "string", "description": "Image path or base64 string"},
                        "bbox": {"type": "string", "description": "Bounding box in format (x1,y1,x2,y2)"},
                        "attribute": {"type": "string", "description": "Attribute to describe, e.g. 'color of the car'"},
                    },
                    "required": ["image", "bbox", "attribute"],
                },
            },
        }
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model-name", type=str, default="RegionAttributeDescription")
    ...
    worker = RegionAttributeDescriptionWorker(
