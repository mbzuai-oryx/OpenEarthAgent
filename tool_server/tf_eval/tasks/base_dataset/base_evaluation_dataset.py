...
class BaseEvalDataset(Dataset):
    ...
    def __init__(
        ...
        if isinstance(self.meta_data, list):
            ...
            print(f"[INFO] Dataset contains {len(self.meta_data)} samples.")
            print(f"[INFO] Keys found across samples: {sorted(all_keys)}")
        elif isinstance(self.meta_data, dict):
            print(f"[INFO] Keys in dataset: {list(self.meta_data.keys())}")
        else:
            print(f"[WARNING] Unexpected data type: {type(self.meta_data)}")
        #------------------------
    ...
    def evaluate(self):
        ...
        pretrained = self.model_args.model_args.get("pretrained", "")
        res = {"task_name": self.task_name, "model_name": pretrained, **res}
        return res
