...
class tp_model(abc.ABC):
    ...
    def set_generation_config(self, generation_configs: dict = None) -> None:
        if generation_configs is not None:
            ...
            self.max_new_tokens = self.generation_config.get("max_length", 2048)
            logger.info(
                f"Generation config updated: max_new_tokens={self.max_new_tokens}, "
                f"full_config={self.generation_config}"
            )
        else:
            self.generation_config = {}
            logger.warning("No generation config provided. Using default settings.")
